...
ForgeQtExtAppShellResources::_start_next_download () {
   ...
   if (_state.downloadQueue.isEmpty ()) {
      ...
      if (_state.current) {
         ...
         if (_state.downloadFile) {

            delete _state.downloadFile; _state.downloadFile = 0;
         }
         ...
         if (_state.downloadFile->open ()) {
            ...
            //       .arg("user").arg("asas").toAscii()).toBase64());
            ...
            if (_state.ui) {
               ...
               String text;
               text << "(" << _state.downloadCount + 1
                    << " of " << _state.totalCount << ")";
...
void
ForgeQtExtAppShellResources::_download_progress (
